...
/**
 * Resolve the go-api-backend base URL. `GO_API_BASE_URL` is the intended
 * server-only name; `NEXT_PUBLIC_API_BASE_URL` is accepted as a fallback so a
 * deployment that only set the public name still works.
 */
function goApiBase(): string | null {
  const raw = process.env.GO_API_BASE_URL ?? process.env.NEXT_PUBLIC_API_BASE_URL;
...
/**
 * Relay an upstream response to the browser. Crucially, if the backend returns a
 * non-JSON body (a bare `404 page not found`, an HTML 502, a platform proxy
 * error), it is wrapped in the standard envelope — otherwise the client's
 * `await res.json()` throws and every failure surfaces as a generic
 * "Network error".
 */
...
/** Temporary compatibility path for WMS routes cut over to go-api-backend. */
